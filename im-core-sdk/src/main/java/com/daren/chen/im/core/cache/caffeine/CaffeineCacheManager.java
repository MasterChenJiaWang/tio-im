 ...
 *
 */
package com.daren.chen.im.core.cache.caffeine;
...
public class CaffeineCacheManager {
        ...
        try {
            ...
            for (CaffeineConfiguration configuration : configurations) {
                register(configuration.getCacheName(), configuration.getTimeToLiveSeconds(),
                    configuration.getTimeToIdleSeconds(), configuration.getMaximumSize(), null);
            }
        } catch (Exception e) {
    ...
    public static CaffeineCache getCache(String cacheName, boolean skipNull) {
        ...
        if (CaffeineCache == null && !skipNull) {
            log.error("cacheName[{}]还没注册，请初始化时调用：{}.register(cacheName, timeToLiveSeconds, timeToIdleSeconds)",
                cacheName, com.daren.chen.im.core.cache.caffeine.CaffeineCache.class.getSimpleName());
        }
    ...
    public static CaffeineCache register(String cacheName, Integer timeToLiveSeconds, Integer timeToIdleSeconds,
                                         Integer maximumSize, RemovalListener<String, Serializable> removalListener) {
        CaffeineCache caffeineCache = map.get(cacheName);
            ...
            synchronized (CaffeineCacheManager.class) {
                ...
                if (caffeineCache == null) {
                    ...
                    LoadingCache<String, Serializable> loadingCache =
                        CaffeineUtils.createLoadingCache(cacheName, timeToLiveSeconds, timeToIdleSeconds,
                            initialCapacity, maximumSize, recordStats, removalListener);
                    LoadingCache<String, Serializable> temporaryLoadingCache = CaffeineUtils.createLoadingCache(
                        cacheName, null, 10, initialCapacity, maximumSize, recordStats, removalListener);
