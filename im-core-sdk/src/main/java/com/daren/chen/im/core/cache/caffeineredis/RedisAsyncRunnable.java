...
import com.daren.chen.im.core.cache.CacheChangeType;
import com.daren.chen.im.core.cache.CacheChangedVo;
...
public class RedisAsyncRunnable implements Runnable {
    ...
    private final LinkedBlockingQueue<RedisL2Vo> redisL2VoQueue = new LinkedBlockingQueue<RedisL2Vo>();
    ...
    public void run() {
        ...
        Map<String, List<Pair<String, Serializable>>> pairMap = new HashMap<>();
        ...
        while (true) {
            try {
                ...
                if (redisL2Vo != null) {
                    ...
                    cacheChangeVos.add(new CacheChangedVo(redisL2Vo.getRedisCache().getCacheName(), redisL2Vo.getKey(),
                        CacheChangeType.PUT).toString());
                    count++;
                }
