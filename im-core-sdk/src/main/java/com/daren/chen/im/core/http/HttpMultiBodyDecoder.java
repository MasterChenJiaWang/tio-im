...
/**
 * @author wchao 2017年7月26日 下午2:20:43
 */
public class HttpMultiBodyDecoder {
    ...
    /**
     * 【 Content-Disposition: form-data; name="uploadFile"; filename="" Content-Type: application/octet-stream 】
     *
     * 【 Content-Disposition: form-data; name="end" 】
     *
     * @author wchao 2017年7月27日 上午10:18:01
     */
        ...
        try {
            ...
            while (true) {
                if (step == Step.BOUNDARY) {
                    String line =
                        ByteBufferUtils.readLine(buffer, request.getCharset(), HttpConfig.MAX_LENGTH_OF_BOUNDARY);
                ...
                if (step == Step.BODY) {
                    Step newParseStep =
                        parseBody(multiBodyHeader, request, buffer, boundary, endBoundary, channelContext);
                    step = newParseStep;
    ...
    public static Step parseBody(Header header, HttpRequest request, ByteBuffer buffer, String boundary,
        String endBoundary, ImChannelContext channelContext)
     ...
     * @author wchao
     */
    public static void parseHeader(List<String> lines, Header header, ImChannelContext channelContext)
        ...
        try {
            ...
            String name =
                HttpParseUtils.getPerprotyEqualValue(header.map, MultiBodyHeaderKey.Content_Disposition, "name");
            String filename =
                HttpParseUtils.getPerprotyEqualValue(header.map, MultiBodyHeaderKey.Content_Disposition, "filename");
