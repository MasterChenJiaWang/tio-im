...
public class TcpServerDecoder implements ImConst {
    ...
    public static TcpPacket decode(ByteBuffer buffer, ImChannelContext imChannelContext) throws ImDecodeException {
        ...
        // byteBuffer的总长度是 =
        // 1byte协议版本号+1byte消息标志位+4byte同步序列号(如果是同步发送则多4byte同步序列号,否则无4byte序列号)+1byte命令码+4byte消息的长度+消息体的长度
        ...
        //
        try {
            if (Command.COMMAND_HEARTBEAT_REQ == tcpPacket.getCommand()) {
                return tcpPacket;
            }
            int number = tcpPacket.getCommand().getNumber();
            byte[] body1 = tcpPacket.getBody();
            if (body1 != null) {
                Object msgNoticeReq = JsonKit.toBean(body1, Object.class);
                if (msgNoticeReq != null) {
                    logger.info("上下文ID [{}] 用户ID [{}]   收到的消息： cmd 【{}】 发送内容 【{}】", imChannelContext.getId(),
                        imChannelContext.getUserId(), number, JSON.toJSONString(msgNoticeReq));
                }
            }
        } catch (Exception e) {
