...
import com.daren.chen.im.server.util.ImServerKit;
...
public class NonPersistentUserInfo implements IUserInfo {
    ...
    public User getUserInfo(UserReqBody userReqBody, ImChannelContext imChannelContext) {
        ...
        if (UserStatusType.ONLINE.getNumber() == type || UserStatusType.OFFLINE.getNumber() == type) {
            ...
            List<User> onlineFriends = initOnlineUserFriends(user.getFriends(), type);
            cloneUser.setFriends(onlineFriends);
            // 处理群组在线用户相关信息;
            // List<Group> onlineGroups = initOnlineUserFriendsGroups(user.getGroups(), type, GROUP_FLAG);
            List<Group> onlineGroups = initOnlineUserGroups(user.getGroups(), type, GROUP_FLAG);
     ...
     * @return
     */
    private static List<User> initOnlineUserFriends(List<User> friends, Integer type) {
     ...
     * @return
     */
    private static List<Group> initOnlineUserGroups(List<Group> groups, Integer type, Integer flag) {
