...
import com.daren.chen.im.core.http.Cookie;
...
import com.daren.chen.im.core.http.listener.IHttpServerListener;
import com.daren.chen.im.core.http.session.HttpSession;
import com.daren.chen.im.server.protocol.http.mvc.Routes;
import com.daren.chen.im.server.util.ClassUtils;
...
public class DefaultHttpRequestHandler implements IHttpRequestHandler, ImConst.Http {
        ...
        try {
            ...
            if (method != null) {
                                    ...
                                    if (ClassUtils.isSimpleTypeOrArray(paramType)) {
                                        ...
                                        for (Entry<String, Object[]> entry : set) {
                                            ...
                                            PropertyDescriptor propertyDescriptor =
                                                BeanUtil.getPropertyDescriptor(paramType, fieldName, true);
                                            if (propertyDescriptor == null) {
                                                ...
                                                if (clazzes == null || clazzes.length != 1) {
                                                    log.info("方法的参数长度不为1，{}.{}", paramType.getName(),
                                                        writeMethod.getName());
                                                    continue label2;
                                                }
                                                ...
                                                if (ClassUtils.isSimpleTypeOrArray(clazz)) {
                                                    if (fieldValue != null && fieldValue.length > 0) {
                                                        if (clazz.isArray()) {
                                                            writeMethod.invoke(paramValues[i],
                                                                Convert.convert(clazz, fieldValue));
                                                        } else {
                                                            writeMethod.invoke(paramValues[i],
                                                                Convert.convert(clazz, fieldValue[0]));
                                                        }
                                                    }
                                                }
                                            }
                ...
                if (obj instanceof HttpResponse) {
                    ...
                    throw new Exception(
                        bean.getClass().getName() + "#" + method.getName() + "返回的对象不是" + HttpResponse.class.getName());
                }
            } else {
    ...
    private void processCookieAfterHandler(HttpRequest request, RequestLine requestLine, HttpResponse httpResponse)
        ...
        HttpSession httpSession = request.getHttpSession();// (HttpSession)
                                                           // channelContext.getAttribute();//.getHttpSession();//not
                                                           // null
    ...
    /**
     * @param staticResCache
     *            the staticResCache to set
     */
