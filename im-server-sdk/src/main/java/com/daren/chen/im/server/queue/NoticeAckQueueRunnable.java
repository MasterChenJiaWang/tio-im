 ...
 */
public class NoticeAckQueueRunnable extends AbstractQueueRunnable<Message> {

    private final Logger log = LoggerFactory.getLogger(NoticeAckQueueRunnable.class);
    ...
    public NoticeAckQueueRunnable(ImChannelContext imChannelContext, Executor executor) {
        super(executor);
    ...
    public void runTask() {
        ...
        while ((message = msgQueue.poll()) != null) {
            if (protocolCmdProcessor != null) {
                protocolCmdProcessor.noticeAck(imChannelContext, message);
            }
        }
    }
